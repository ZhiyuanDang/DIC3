...
def create_config(config_file_env, config_file_exp):
    ...
    cfg['centroids_feature_path'] = os.path.join(pretext_dir, 'centroids_features.npy')
    ...
    if cfg['setup'] in ['ccdc', 'selflabel']:
        base_dir = os.path.join(root_dir, cfg['train_db_name'])
        ccdc_dir = os.path.join(base_dir, 'ccdc')
        selflabel_dir = os.path.join(base_dir, 'selflabel') 
        mkdir_if_missing(base_dir)
        mkdir_if_missing(ccdc_dir)
        mkdir_if_missing(selflabel_dir)
        cfg['ccdc_dir'] = ccdc_dir
        cfg['ccdc_checkpoint'] = os.path.join(ccdc_dir, 'checkpoint.pth.tar')
        cfg['ccdc_model'] = os.path.join(ccdc_dir, 'model.pth.tar')
