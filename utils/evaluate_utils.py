...
def contrastive_evaluate(val_loader, model, memory_bank):
    ...
    for batch in val_loader:
        ...
        output = model(images, withhead = False)
...
def scan_evaluate(predictions):
    # Evaluate model based on CCDC loss.
    ...
    for head in predictions:
        ...
        probs = F.normalize(probs, dim = 0)
        similarity = torch.matmul(probs.t(), probs).cuda()
        labels = torch.tensor(list(range(probs.shape[1]))).cuda()
        ce_loss = torch.nn.CrossEntropyLoss()(similarity, labels).item()
        
        # Total loss
        total_loss = - entropy_loss + ce_loss
        
        output.append({'entropy': entropy_loss, 'ce': ce_loss, 'total_loss': total_loss})
    ...
    return {'ccdc': output, 'lowest_loss_head': lowest_loss_head, 'lowest_loss': lowest_loss}
...
def hungarian_evaluate(subhead_index, all_predictions, class_names=None, 
    ...
    num_classes = torch.unique(targets, dim=0).numel()
...
@torch.no_grad()
def hungarian_evaluate_kmeans(subhead_index, all_predictions, class_names=None, 
                        compute_purity=True, compute_confusion_matrix=True,
                        confusion_matrix_file=None):
    # Evaluate model based on hungarian matching between predicted cluster assignment and gt classes.
    # This is computed only for the passed subhead index.
