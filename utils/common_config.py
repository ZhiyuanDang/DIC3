...
def get_model(p, pretrain_path=None):
    ...
    if p['backbone'] == 'resnet18':
        ...
        elif p['train_db_name'] in ['stl-10', 'imagenet_10', 'imagenet_dogs']:
        ...
        if 'imagenet' in p['train_db_name']:
            ...
            backbone = resnet50()  
        elif p['train_db_name'] in ['cifar-10', 'cifar-20']:
            from models.resnet import resnet50
            backbone = resnet50()  
    ...
    elif p['setup'] in ['ccdc', 'selflabel']:
        from models.models import ClusteringModel
        if p['setup'] == 'selflabel':
            assert(p['num_heads'] == 1)
        model = ClusteringModel(backbone, p['num_classes'], p, p['num_heads'])
    ...
    if pretrain_path is not None and os.path.exists(pretrain_path):
        ...
        if p['setup'] == 'ccdc': # Weights are supposed to be transfered from contrastive training
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
                        to_neighbors_dataset=False, split=None, tr_te_merge = False):
    # Base dataset
    if p['train_db_name'] == 'cifar-10':
        from data.cifar import CIFAR10
        dataset = CIFAR10(train=True, transform=transform, download=True)
        # if tr_te_merge:
        #     dataset_val = CIFAR10(train=False, transform=transform, download=True)
        #     dataset = ConcatDataset([dataset, dataset_val])
        ...
        # if tr_te_merge:
        #     dataset_val = CIFAR20(train=False, transform=transform, download=True)
        #     dataset = ConcatDataset([dataset, dataset_val])
            # dataset.transform = transform
        ...
        # if tr_te_merge:
        #     dataset_val = STL10(split='test', transform=transform, download=True)
        #     dataset = ConcatDataset([dataset, dataset_val])
            # dataset.transform = transform
    
    elif p['train_db_name'] == 'tiny-imagenet':
        from data.imagenet import TinyImageNet
        subset_file = './data/imagenet_subsets/words.txt'
        dataset = TinyImageNet(subset_file=subset_file, split='train', transform=transform)

    elif p['train_db_name'] == 'imagenet':
        from data.imagenet import ImageNet
        dataset = ImageNet(split='train', transform=transform)

    elif p['train_db_name'] in ['imagenet_50', 'imagenet_100', 'imagenet_200', 'imagenet_10', 'imagenet_dogs']:
        ...
        # dataset_val = ImageNetSubset(subset_file=subset_file, split='val', transform=transform)
        # dataset = ConcatDataset([dataset_train, dataset_val])
        # dataset.transform = dataset_train.transform
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False, to_augmented_dataset = False, tr_te_merge = False):
    # Base dataset
    if p['val_db_name'] == 'cifar-10':
        from data.cifar import CIFAR10
        dataset = CIFAR10(train=False, transform=transform, download=True)
        # if tr_te_merge:
        #     dataset_val = CIFAR10(train=True, transform=transform, download=True)
        #     dataset = ConcatDataset([dataset, dataset_val])
            # dataset.transform = transform
        ...
        # if tr_te_merge:
        #     dataset_val = CIFAR20(train=True, transform=transform, download=True)
        #     dataset = ConcatDataset([dataset, dataset_val])
            # dataset.transform = transform
        ...
        # if tr_te_merge:
        #     dataset_val = STL10(split='train', transform=transform, download=True)
        #     dataset = ConcatDataset([dataset, dataset_val])
            # dataset.transform = transform
...
def get_train_transformations(p):
    if p['augmentation_strategy'] == 'standard':
        ...
        # Augmentation strategy from our paper data_transforms = 
        # return  transforms.Compose([
        #     transforms.RandomHorizontalFlip(),
