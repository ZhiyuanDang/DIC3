...
def main():
    ...
    memory_bank_base = MemoryBank(len(base_dataset), 
                                model.module.contrastive_head[0].weight.shape[0],
    ...
    elif os.path.exists(p['pretext_model']):
        print(colored('Restart from model {}'.format(p['pretext_model']), 'blue'))
        checkpoint = torch.load(p['pretext_model'], map_location='cpu')
    ...
    centroids_features = memory_bank_base.run_kmeans()
    print('Computing the centroids ...')
    print('The shape of centroids is')
    print(np.shape(centroids_features))
    np.save(p['centroids_feature_path'], centroids_features)   
    ...
    memory_bank_val.run_test_kmeans(centroids_features) 
