...
class ImageNetSubset(data.Dataset):
    def __init__(self, subset_file, root=MyPath.db_root_dir('imagenet'), split='train', 
        ...
        if split == 'train':
            ...
            self.root = os.path.join(root, 'ILSVRC2012_img_%s_with_subfolders' %(split))
        self.transform = transform
        self.split = split
...
class TinyImageNet(data.Dataset):
    def __init__(self, subset_file, root=MyPath.db_root_dir('tiny-imagenet'), split='train', 
                    transform=None):
